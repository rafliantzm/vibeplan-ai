  ...
  return (
    <div className="grid gap-5">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
        <div className="grid min-h-[calc(100vh-140px)] lg:h-[calc(100vh-150px)] lg:min-h-[640px] lg:grid-cols-[360px_minmax(0,1fr)]">
          <aside
            className={[
              "flex min-h-0 flex-col bg-[linear-gradient(180deg,_#ffffff_0%,_#f8fafc_100%)] border-slate-200 lg:border-b-0 lg:border-r",
              isMobileRoomOpen ? "hidden lg:block" : "block border-b",
            ].join(" ")}
          >
            <div className="shrink-0 border-b border-slate-100 bg-white/95 px-5 py-5 backdrop-blur">
              ...
              <form onSubmit={handleApplyFilters} className="mt-5 grid gap-3">
                <input
                  name="search"
          ...
          <div
            className={[
              "min-h-0 flex-col bg-[linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)]",
          ...
          >
            {selectedConversation ? (
              <>
                ...
                <div
                  ref={messageListRef}
                  className="min-h-0 flex-1 space-y-4 overflow-y-auto bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.08),_transparent_30%),linear-gradient(180deg,_#f8fafc_0%,_#eef2ff_100%)] px-5 py-5"
                >
                  {isLoadingDetail ? (
                  ...
                  {messages.map((item) => (
                    <ChatBubble
                      ...
                      senderName={item.sender_name || (item.sender_type === "admin" ? "Admin" : selectedConversation.name)}
...
function mergeConversationList(current, nextConversation) {
  ...
  const nextItems = current.some((item) => item.id === nextConversation.id)
    ? current.map((item) => (item.id === nextConversation.id ? { ...item, ...nextConversation } : item))
